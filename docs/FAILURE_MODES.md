...
| OF-08 | **Governance amplification:** a small product change triggers repeated full audits, new evidence machinery, new benchmark infrastructure or release-grade provenance without a matching risk increase. Delivery slows while the control surface grows. | Classify the change PRACTICAL/REVIEWED/STRICT first. Use the cheapest evidence that can falsify the relevant failure mode; repeat full verification only when earlier evidence is invalidated. Move optional process improvements to backlog. | Compare product-change scope with governance work, repeated checks and newly introduced control surface. Record the risk premise that justified any escalation. |
| OF-09 | **Scope-expansion blocker:** a verifier finds a useful improvement outside current acceptance and turns it into mandatory work, so one FIFO item recursively creates more blockers. | Block only failed acceptance, a relevant required gate, material changed-scope risk or invalid required evidence. Record other findings as deferred work with a pointer to the discovery. | The blocking finding names the violated acceptance/gate/risk/evidence boundary. Deferred findings remain visible but do not prevent completion. |
